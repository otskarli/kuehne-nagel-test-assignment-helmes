...
it('returns unique category length', async () => {
    const productService = new ProductService();
    const categories = await productService.listCategories();
    expect(categories).toHaveLength(5);
});

it('returns empty list for no search terms specified', async () => {
    const productService = new ProductService();
...
it('returns products with only one category selected', async () => {
    const productService = new ProductService();
    const products = await productService.list({
        name: null,
...
it('returns products with only one category selected and name entered', async () => {
    const productService = new ProductService();
    const products = await productService.list({
        name: "JetBrain",
        categories: ["Software Development"]
...
it('returns products with three categories selected and name entered', async () => {
    const productService = new ProductService();
    const products = await productService.list({
        name: "2020",
        categories: ["Software Development", "Management Tools", "Text Editors"]
    });
    expect(products).toHaveLength(2);
});